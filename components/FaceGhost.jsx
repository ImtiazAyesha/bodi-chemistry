...
const FaceGhost = ({ isAligned, holdDuration = 0, stage1Debug = null }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            {isAligned ? '✓ Perfect! Hold Position' : (feedbackMessage || 'Align Face with Outline')}
                ...
                <svg width="18" height="18" viewBox="0 0 20 20">
                  <g transform="translate(10, 10)">
                    ...
                    {(feedbackMessage.toLowerCase().includes('closer')) && (
                      <path d="M -7 0 L -1 -4 L -1 -1 L 1 -1 L 1 -4 L 7 0 L 1 4 L 1 1 L -1 1 L -1 4 Z" fill={guidanceColor}>
                        <animate attributeName="opacity" values="0.7;1;0.7" dur="1.2s" repeatCount="indefinite" />
                      </path>
                    )}
                    {(feedbackMessage.toLowerCase().includes('back')) && (
                      <g>
                        <path d="M -1 0 L -7 -4 L -7 -1 L -9 -1 L -9 1 L -7 1 L -7 4 Z" fill={guidanceColor} />
                        <path d="M 1 0 L 7 -4 L 7 -1 L 9 -1 L 9 1 L 7 1 L 7 4 Z" fill={guidanceColor} />
                        <animate attributeName="opacity" values="0.7;1;0.7" dur="1.2s" repeatCount="indefinite" />
                      </g>
                    )}
      ...
      {/* SVG Face Outline - Perfectly centered & viewport-safe */}
      <svg
        style={{
          position: 'absolute',
          top: '50%',
      ...
      >
        <defs>
          {/* Viewport-Safe Clip: Keeps the head prominently visible while preventing overflow */}
          <clipPath id="head-mask">
            <ellipse cx="0" cy="-80" rx="200" ry="260" />
          </clipPath>
        </defs>

        {/* FACE SILHOUETTE - Positioned to avoid banner overlap (y=580) */}
        <g transform="translate(240, 580)" clipPath="url(#head-mask)">

          {/* Centered Glow effect */}
          {isAligned && (
            <ellipse cx="0" cy="-80" rx="220" ry="280"
              fill="none" stroke={successColor}
              strokeWidth="28" opacity="0.12"
              filter="blur(28px)"
            />
          )}
