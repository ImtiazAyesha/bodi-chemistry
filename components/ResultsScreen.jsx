...
const ResultsScreen = ({ captureData, questionnaireData, patternResults, onRestart }) => {
  ...
  return (
    <div className="min-h-screen bg-brand-sand text-brand-slate overflow-y-auto overflow-x-hidden relative selection:bg-brand-sage/30 pb-20">
      {/* Background Layers */}
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1 rounded-full bg-white/40 border border-brand-sage/20 shadow-sm mb-6">
            <span className="w-2 h-2 rounded-full bg-brand-sage animate-pulse" />
            <span className="text-[10px] font-display font-bold text-brand-deepSage tracking-[0.2em] uppercase">
              Analysis Complete
            </span>
          </div>

          <h1 className="text-3xl sm:text-4xl md:text-6xl font-display font-bold text-brand-slate tracking-tighter mb-4">
            BODI <span className="text-brand-deepSage">KEMISTRI</span> REPORT
          </h1>
          <p className="text-brand-slate/60 font-medium tracking-wide uppercase text-[10px]">Precision Postural Biometrics</p>
        </motion.div>
        ...
        >
          {/* Main Score */}
          <div className="md:col-span-3 lg:col-span-1 bg-white border border-brand-sage/10 rounded-3xl p-6 sm:p-10 text-center relative overflow-hidden group shadow-glass">
            ...
            <h3 className="text-brand-deepSage/60 font-display font-semibold text-[10px] uppercase tracking-[0.2em] mb-4">Overall Wellness Score</h3>
            <div className="text-7xl sm:text-8xl md:text-9xl font-display font-bold text-brand-slate tracking-tighter mb-4">
              {score.total}
            </div>
            <div className="h-1.5 w-20 sm:w-24 mx-auto bg-brand-sage/20 rounded-full" />
          </div>

          {/* Sub Scores */}
          <div className="md:col-span-3 lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            <div className="bg-white/60 border border-brand-sage/10 rounded-3xl p-6 sm:p-8 flex flex-col justify-center items-center backdrop-blur-md hover:border-brand-sage/30 transition-all shadow-glass group">
              <span className="text-[10px] font-display font-semibold text-brand-sage mb-4 tracking-[0.2em] uppercase">FACE METRICS</span>
              <span className="text-4xl sm:text-5xl font-display font-bold text-brand-slate group-hover:text-brand-deepSage transition-colors">{score.face}</span>
              <div className="mt-4 w-10 sm:w-12 h-1 bg-brand-sage/10 rounded-full" />
            </div>
            <div className="bg-white/60 border border-brand-sage/10 rounded-3xl p-6 sm:p-8 flex flex-col justify-center items-center backdrop-blur-md hover:border-brand-sage/30 transition-all shadow-glass group">
              <span className="text-[10px] font-display font-semibold text-brand-slate/60 mb-4 tracking-[0.2em] uppercase">BODY ALIGNMENT</span>
              <span className="text-4xl sm:text-5xl font-display font-bold text-brand-slate group-hover:text-brand-deepSage transition-colors">{score.body}</span>
              <div className="mt-4 w-10 sm:w-12 h-1 bg-brand-sage/10 rounded-full" />
            </div>
          </div>
        </motion.div>
        ...
        <div className="mb-20">
          <div className="flex items-center gap-4 mb-6 sm:mb-10">
            <div className="w-1.5 h-6 sm:h-8 bg-brand-slate rounded-full" />
            <h3 className="text-xl sm:text-2xl font-display font-bold text-brand-slate uppercase tracking-tight">
              Captured Evidence
            </h3>
          </div>
                ...
                <div className="p-6 bg-white">
                  <div className="space-y-3">
                    {item.metrics.map((m, idx) => (
                      <div key={idx} className="flex justify-between text-[11px] font-display font-semibold border-b border-brand-sand pb-2 last:border-0 last:pb-0">
                        <span className="text-brand-deepSage/60 uppercase racking-wider">{m.l}</span>
        ...
        {patternResults && patternResults.allPatterns && (
          <div className="mb-24">
            <div className="text-center mb-10 sm:mb-16">
              <h2 className="text-2xl sm:text-3xl md:text-5xl font-display font-bold text-brand-slate mb-4">Somatic Pattern Analysis</h2>
              <p className="text-brand-deepSage/70 font-medium max-w-2xl mx-auto leading-relaxed text-sm sm:text-base px-4">
            ...
            {patternResults.primaryPattern && (
              <motion.div
                ...
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                className="relative bg-white border border-brand-sage/20 rounded-[32px] sm:rounded-[40px] p-8 sm:p-16 mb-16 text-center overflow-hidden shadow-brand"
              >
                <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-transparent via-brand-sage to-transparent" />

                <div className="relative z-10 flex flex-col items-center">
                  <div className="inline-flex items-center justify-center w-20 h-20 sm:w-24 sm:h-24 rounded-[1.5rem] sm:rounded-[2rem] bg-black mb-6 sm:mb-8 text-white shadow-2xl">
                    <FiZap className="w-8 h-8 sm:w-10 sm:h-10 fill-current" />
                  </div>

                  <h3 className="text-2xl sm:text-3xl md:text-4xl font-display font-bold text-brand-slate mb-4">
                  ...
                  <div className="flex flex-wrap justify-center gap-4 sm:gap-6 mt-8 sm:mt-10">
                    <div className="px-4 sm:px-6 py-3 sm:py-4 rounded-2xl sm:rounded-3xl bg-brand-sand/50 border border-brand-sage/10 text-brand-slate flex flex-col items-center min-w-[120px] sm:min-w-[140px]">
                      <span className="text-[10px] font-display font-medium uppercase tracking-widest text-brand-deepSage/60 mb-1">Severity</span>
                      <span className="text-lg sm:text-xl font-display font-semibold uppercase text-brand-sage">{patternResults.primaryPattern.severity}</span>
                    </div>
                    <div className="px-4 sm:px-6 py-3 sm:py-4 rounded-2xl sm:rounded-3xl bg-brand-sand/50 border border-brand-sage/10 text-brand-slate flex flex-col items-center min-w-[120px] sm:min-w-[140px]">
                      <span className="text-[10px] font-display font-medium uppercase tracking-widest text-brand-deepSage/60 mb-1">Pattern Match</span>
                      <span className="text-lg sm:text-xl font-display font-semibold text-brand-slate">{patternResults.primaryPattern.score.toFixed(0)}%</span>
                    </div>
                    <div className="px-4 sm:px-6 py-3 sm:py-4 rounded-2xl sm:rounded-3xl bg-brand-sand/50 border border-brand-sage/10 text-brand-slate flex flex-col items-center min-w-[120px] sm:min-w-[140px]">
                      <span className="text-[10px] font-display font-medium uppercase tracking-widest text-brand-deepSage/60 mb-1">Confidence</span>
                      <span className="text-lg sm:text-xl font-display font-semibold text-brand-slate">{patternResults.confidence.percentage}%</span>
                    </div>
                  </div>
                </div>
              </motion.div>
        ...
        {/* Action Buttons */}
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center items-center pb-20 px-4">
          <motion.button
            whileHover={{ scale: 1.02, backgroundColor: "#E2DACF" }}
            whileTap={{ scale: 0.98 }}
            onClick={onRestart}
            className="group px-8 sm:px-10 py-4 sm:py-5 rounded-2xl bg-transparent border border-brand-slate/20 text-brand-slate font-display font-semibold text-xs uppercase tracking-[0.2em] transition-all duration-300 w-full sm:min-w-[260px] sm:w-auto flex items-center justify-center gap-4 shadow-sm"
          >
          ...
          <motion.button
            ...
            className="group px-8 sm:px-10 py-4 sm:py-5 rounded-2xl bg-transparent border border-brand-slate/20 text-brand-slate font-display font-semibold text-xs uppercase tracking-[0.2em] transition-all duration-300 w-full sm:min-w-[260px] sm:w-auto flex items-center justify-center gap-4 shadow-sm hover:text-white"
          >
